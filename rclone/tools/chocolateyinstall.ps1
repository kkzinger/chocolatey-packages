...
$packageName= 'rclone' # arbitrary name for the package, used in messages
$toolsDir   = "$(Split-Path -parent $MyInvocation.MyCommand.Definition)"
$url        = 'http://downloads.rclone.org/rclone-v1.34-windows-386.zip' # download url, HTTPS preferred
$url64      = 'http://downloads.rclone.org/rclone-v1.34-windows-amd64.zip' # 64bit URL here (HTTPS preferred) or remove - if installer contains both (very rare), use $url
#$fileLocation = Join-Path $toolsDir 'rclone-v1.34-windows-amd64.zip'
...
$packageArgs = @{
  ...
  softwareName  = 'rclone*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
